...
for file in $files; do
    ...
    if [ $? -ne 0 ]; then
        sdiff -s <(./abi-check "master/Server/$file" --incl ".*/SDK/include/.*" --excl ".*/Impl/.*") <(./abi-check "ref/Server/$file" --incl ".*/SDK/include/.*" --excl ".*/Impl/.*")
        ...
        if [ $? -ne 0 ]; then
            ...
            echo "$res" | grep -G -m 1 '^<'
            if [ $? -eq 0 ]; then
                sdiff -s <(./abi-check "master/Server/$file" --incl ".*/SDK/include/.*" --excl ".*/Impl/.*" --no-names) <(./abi-check "ref/Server/$file" --incl ".*/SDK/include/.*" --excl ".*/Impl/.*" --no-names)
                ret=1
