...
uint16_t* AMXAPI amx_Align16(uint16_t* v) { return PawnImpl::Align16(v) } uint32_t* AMXAPI amx_Align32(uint32_t* v) { return PawnImpl::Align32(v) }

#if defined _I64_MAX || defined HAVE_I64
uint64_t* AMXAPI amx_Align64(uint64_t* v)
{
	return PawnImpl::Align64(v)
}
#endif

int AMXAPI amx_Allot(AMX* amx, int cells, cell* amx_addr, cell** phys_addr)
{
	return PawnImpl::Allot(amx, cells, amx_addr, phys_addr);
}
